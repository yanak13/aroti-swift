...
struct AstrologyDetailSheet: View {
    ...
    let blueprint: AstrologyBlueprint
    @State private var selectedPlacement: PlanetaryPlacement?
    @State private var showPlacementDetail = false
    ...
    var body: some View {
        NavigationStack {
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(alignment: .leading, spacing: DesignSpacing.md) {
                        ...
                        VStack(alignment: .leading, spacing: DesignSpacing.sm) {
                            Text("What Your Sun Represents")
                                .font(DesignTypography.headlineFont(weight: .semibold))
                                .foregroundColor(DesignColors.foreground)
                            
                            Text(getPlanetDescription(for: "sun"))
                        ...
                        VStack(alignment: .center, spacing: DesignSpacing.sm) {
                            ...
                            let traits = getKeyTraits(for: blueprint.sun.planet, sign: blueprint.sun.sign)
                        ...
                        BaseCard {
                            VStack(alignment: .leading, spacing: DesignSpacing.sm) {
                                ...
                                Text(getManifestationText(for: blueprint.sun.planet, sign: blueprint.sun.sign))
                                    .font(DesignTypography.bodyFont())
                                    .foregroundColor(DesignColors.mutedForeground)
                                    .lineSpacing(4)
                            }
                        ...
                        HStack(alignment: .top, spacing: DesignSpacing.md) {
                            ...
                            VStack(alignment: .center, spacing: DesignSpacing.sm) {
                                ...
                                VStack(alignment: .center, spacing: DesignSpacing.xs) {
                                    ForEach(getStrengths(for: blueprint.sun.planet, sign: blueprint.sun.sign), id: \.self) { strength in
                                        Text("• \(strength)")
                                            .font(DesignTypography.bodyFont())
                                            .foregroundColor(DesignColors.mutedForeground)
                                    }
                            ...
                            VStack(alignment: .center, spacing: DesignSpacing.sm) {
                                ...
                                VStack(alignment: .center, spacing: DesignSpacing.xs) {
                                    ForEach(getChallenges(for: blueprint.sun.planet, sign: blueprint.sun.sign), id: \.self) { challenge in
                                        Text("• \(challenge)")
                                            .font(DesignTypography.bodyFont())
                                            .foregroundColor(DesignColors.mutedForeground)
                                    }
                        ...
                        BaseCard {
                            VStack(alignment: .leading, spacing: DesignSpacing.sm) {
                                ...
                                Text(getPracticalInsights(for: blueprint.sun.planet, sign: blueprint.sun.sign))
                                    .font(DesignTypography.bodyFont())
                                    .foregroundColor(DesignColors.mutedForeground)
                                    .lineSpacing(4)
                            }
                        ...
                        BaseCard {
                            VStack(alignment: .leading, spacing: DesignSpacing.sm) {
                                Text("About \(blueprint.sun.sign)")
                                    .font(DesignTypography.headlineFont(weight: .semibold))
                                    .foregroundColor(DesignColors.foreground)
                                
                                Text(getSignInfo(for: blueprint.sun.sign))
                                    .font(DesignTypography.bodyFont())
                                    .foregroundColor(DesignColors.mutedForeground)
                                    .lineSpacing(4)
                            }
                        ...
                        VStack(alignment: .leading, spacing: DesignSpacing.sm) {
                            Text("Premium Placements")
                                .font(DesignTypography.headlineFont(weight: .semibold))
                                .foregroundColor(DesignColors.foreground)
                            
                            Text("Unlock access to Moon, Rising, and all other planetary placements")
                                .font(DesignTypography.footnoteFont())
                                .foregroundColor(DesignColors.mutedForeground)
                            
                            // Premium placements list (locked)
                            VStack(spacing: DesignSpacing.xs) {
                                // Moon
                                PremiumPlacementRow(placement: blueprint.moon)
                                
                                // Rising
                                PremiumPlacementRow(placement: blueprint.rising)
                                
                                // Other planets if available
                                if let venus = blueprint.venus {
                                    PremiumPlacementRow(placement: venus)
                                }
                                if let mars = blueprint.mars {
                                    PremiumPlacementRow(placement: mars)
                                }
                                if let mercury = blueprint.mercury {
                                    PremiumPlacementRow(placement: mercury)
                                }
                            }
                        }
                    }
            ...
            .sheet(isPresented: $showPlacementDetail) {
                if let placement = selectedPlacement {
                    AstrologyPlacementDetailView(placement: placement)
                }
            }
        }
    }
...
// MARK: - Premium Placement Row
struct PremiumPlacementRow: View {
...
// MARK: - Placement Detail View
struct AstrologyPlacementDetailView: View {
    @Environment(\.dismiss) var dismiss
    let placement: PlanetaryPlacement
